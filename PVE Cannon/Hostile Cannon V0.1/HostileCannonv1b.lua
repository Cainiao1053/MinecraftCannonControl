...
function getFlatTarget(posx,posz,posy,xtgt,ztgt,ytgt,C0,C1,minf,maxf,selfVelocity,vxtgt,vztgt) --should run after getting Cs and limits, angles are in deg
    ...
    local yaw = math.atan2(-(xtgt-posx+(vxtgt-selfVelocity.x)*tShell),((ztgt-posz+(vztgt-selfVelocity.z)*tShell)));
    yaw = yaw*180/math.pi
    if yaw<-180 then
        yaw = yaw +360;
    elseif yaw>180 then
            ...
            if EXEMODE == 0 then
                ...
                for key, val in pairs(getTargetShips) do
                    ...
                    for i = 1,shipWLNum do
                        if string.find(val.slug,"HostileTurret",1, true) ~= nil then
                            isWL = 1;
                        end
                    end
                    if isWL == 0 then
...
function AutoFiring()
    ...
    while true do
    if EXEMODE ~= 0 then
        if math.abs(shipyaw-yawAngle) < 3 and math.abs(g[1].getPitch()-pitchAngle) < 2 then
            for i = 1, gNum do
                g[i].fire();
                randh = math.random(-100,100)/20;
                randy = math.random(-100,100)/50;
            end
            sleep(7);
        end
    end
...
pitchAngle = pitchAngle+shipPitchFix+verfix+randy;
pitchAngle = pitchLimit(pitchAngle,60,-10);
yawAngle = yawAngle + horfix + randh;
...
gb2.setTargetSpeed(speedYaw);
    ...
    gb1.setTargetSpeed(0);
    gb2.setTargetSpeed(0);
    sleep(1);
end
sleep(0.05)
end
